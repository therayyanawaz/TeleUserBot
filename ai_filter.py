...
def _resolve_groq_url() -> str:
    url = str(getattr(config, "GROQ_BASE_URL", DEFAULT_GROQ_BASE_URL) or "").strip()
    return url or DEFAULT_GROQ_BASE_URL


def _resolve_groq_model() -> str:
    model = str(getattr(config, "GROQ_MODEL", DEFAULT_GROQ_MODEL) or "").strip()
...
def _extract_groq_content(payload: dict) -> str:
    try:
        choices = payload.get("choices")
        if not isinstance(choices, list) or not choices:
            return ""
...
def _raise_groq_http_error(response: curl_requests.Response) -> None:
    ...
    try:
        ...
        pass
    if response.status_code in (401, 403):
        raise _CodexAuthError(message or f"Groq HTTP {response.status_code}")
    if response.status_code == 429:
        _record_quota_429()
        raise _CodexRateLimitError(message or "Groq rate limit reached.")
    raise _CodexApiError(message or f"Groq API error: HTTP {response.status_code}")
...
def _filter_decision_system_prompt() -> str:
    ...
    prompt = (
        ...
        "Use skip for conversational fragments, vague rumors lacking concrete attribution, out-of-context replies, or sentences starting with 'But we know that', 'I think', 'Some say'.\n"
...
class _StreamingGroqResponse:
    ...
    async def __aenter__(self) -> "_StreamingGroqResponse":
        ...
        if self._response.status_code >= 400:
            ...
            await self.__aexit__(None, None, None)
            _raise_groq_http_error(error_response)
        self.metrics.started_at = time.time()
        return self
    ...
    *,
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            if attempt < 2:
                LOGGER.warning(
                    "Groq transport error on attempt %s/3: %s",
                    attempt + 1,
                    exc,
                )
                await asyncio.sleep(0.5 * (attempt + 1))
                continue
            raise _CodexApiError(f"Groq request failed after retries: {exc}") from exc
    if response is None:
        raise _CodexApiError("Groq request failed before receiving a response.")
    if response.status_code >= 400:
        _raise_groq_http_error(response)
    try:
        data = response.json()
    except (json.JSONDecodeError, ValueError) as exc:
        LOGGER.debug("Failed to parse Groq JSON response: %s", exc)
        return ""
    return _extract_groq_content(data)
...
async def _call_groq(
    ...
    *,
    ...
    if _streaming_enabled() and on_token is not None:
        try:
            fragments: list[str] = []
            LOGGER.debug("LLM provider=groq model=%s stream=true", _resolve_groq_model())
            async with _StreamingGroqResponse(
                cleaned=cleaned,
                instructions=instructions,
                image_data_urls=image_data_urls,
            ) as stream:
            ...
            LOGGER.debug("Groq streaming failed, falling back to non-stream: %s", exc)
    return await _call_groq_non_stream(
        cleaned,
        instructions,
        verbosity=verbosity,
        image_data_urls=image_data_urls,
